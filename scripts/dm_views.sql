...
FROM s_store_returns
    ...
    LEFT OUTER JOIN time_dim ON (
        (
            cast(substr(sret_return_time, 1, 2) AS signed) * 3600 + cast(substr(sret_return_time, 4, 2) AS signed) * 60 + cast(substr(sret_return_time, 7, 2) AS signed)
        ) = t_time
    )
...
FROM s_web_returns
    ...
    LEFT OUTER JOIN time_dim ON (
        (
            CAST(SUBSTR(wret_return_time, 1, 2) AS signed) * 3600 + CAST(SUBSTR(wret_return_time, 4, 2) AS signed) * 60 + CAST(SUBSTR(wret_return_time, 7, 2) AS signed)
        ) = t_time
    )
...
FROM s_catalog_returns
    ...
    LEFT OUTER JOIN time_dim ON (
        (
            CAST(substr(cret_return_time, 1, 2) AS signed) * 3600 + CAST(substr(cret_return_time, 4, 2) AS signed) * 60 + CAST(substr(cret_return_time, 7, 2) AS signed)
        ) = t_time
    )
