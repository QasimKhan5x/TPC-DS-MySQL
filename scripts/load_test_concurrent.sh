...
load_data() {
    ...
    "
    # multithreading
    # mysqlimport --host=mysql --local --fields-terminated-by=',' --lines-terminated-by='\n' --use-threads=4 --databases $DB $file
    
    end_time=$(date +%s.%N)  # end time for this table
    elapsed_time=$(echo "$end_time - $start_time" | bc)
    
    echo "$table_name:$elapsed_time"
}
parallel load_data ::: $DATA_DIR/*.csv > results/load_times_$exp_id.txt
